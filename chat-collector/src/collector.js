...
export async function startCollector() {
    console.log(`[collector] Connecting to ${config.chatWsUrl}...`);
    chat = new WsChat(config.chatWsUrl);

    chat.on(WsChatEvents.open, () => {
        console.log('[collector] Connected to chat server');
    });

    chat.on(WsChatEvents.close, () => {
        console.log('[collector] Disconnected from chat server');
        stopFlushTimer();
        scheduleReconnect();
    });
    ...
    chat.on(WsChatEvents.error, (err) => {
        console.error('[collector] Chat error:', err);
    });
    ...
    chat.on(WsChatEvents.userStatusChanged, (room, userobj) => {
    ...
    try {
        ...
        console.log('[collector] Authenticated successfully');

        for (const roomTarget of config.chatRooms) {
            const room = await chat.joinRoom(roomTarget, { autoLogin: true, loadHistory: true });
            console.log(`[collector] Joined room: ${room.target}`);
        }

        startFlushTimer();
    } catch (err) {
        console.error('[collector] Failed to initialize:', err);
        scheduleReconnect();
    }
}
...
function scheduleReconnect() {
    ...
    const delay = 10000;
    console.log(`[collector] Reconnecting in ${delay / 1000}s...`);
    reconnectTimeout = setTimeout(async () => {
        reconnectTimeout = null;
        try {
            await startCollector();
        } catch (err) {
            console.error('[collector] Reconnect failed:', err);
            scheduleReconnect();
        }
    }, delay);
}
