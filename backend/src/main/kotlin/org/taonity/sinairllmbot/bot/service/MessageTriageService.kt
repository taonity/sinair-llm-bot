...
class MessageTriageService(
    ...
    fun assess(roomTarget: String): TriageVerdict {
        ...
        val system = buildString {
            ...
            append("knowledge.\n")
            append("4) needsRepoLookup (boolean): would answering well require reading source code, ")
            append("configuration, deployment manifests, CI/CD or infrastructure files, or repository ")
            append("history? The bot may investigate any public GitHub repository, plus every ")
            append("repository in the 'taonity' organization. The project that ")
            append("created this bot is taonity/sinair-llm-bot. Say TRUE for any question about this ")
            append("bot's implementation, behavior, infrastructure, deployment, hosting, build, ")
            append("CI/CD pipeline, Docker setup, Docker Compose, Maven/module structure, ")
            append("database schema, migrations, monitoring, alerting, or any other technical ")
            append("operations detail — including how it works, is deployed, configured, ")
            append("or why it behaves a certain way. Also say TRUE when the latest message asks how the ")
            append("app or any repo is implemented, what a specific file, class, function, endpoint, ")
            append("setting or config value does, why a part of the codebase ")
            append("behaves a certain way, or what changed recently in the code. As a rule of thumb, ")
            append("if the message touches a server-side, infrastructure, or technical-operations ")
            append("topic, say TRUE. Say FALSE only for ")
