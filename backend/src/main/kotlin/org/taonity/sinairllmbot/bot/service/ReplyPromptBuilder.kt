...
class ReplyPromptBuilder(
    ...
    fun build(roomTarget: String, trigger: ChatMessageEntity): ReplyPrompt {
        ...
        val userText = buildString {
            ...
            append("apply the same recipient and acknowledgement rules to this handoff. ")
            append("Check the actual recipient and context before using tools. ")
            append("Return {\"lead\":\"\",\"blocks\":[]} when the message is clearly only discussing or ")
            append("quoting you without speaking to you, addressed only to another person, already fully ")
            append("answered, withdrawn or superseded, or a bare acknowledgement with no pending action. ")
            append("Otherwise answer genuine direct address, including greetings and jokes. Your name or ")
            append("alias anywhere in the message, with or without @, can address you; no explicit invitation ")
            append("is required. Mentioning another person as the subject does not make them the recipient. ")
            append("When direct address is plausible and the exclusion is uncertain, answer. Do not choose ")
            append("silence merely because a response is social, subjective, simple or helpful. ")
            append("Answer unresolved questions to the room when you can contribute, including opinions ")
            append("and recommendations. A question need not name you or have a question mark. ")
            append("Accepting your offer to act is not a bare acknowledgement. ")
            append("Also fulfill a direct request to answer, continue or retry; do not treat it as noise ")
