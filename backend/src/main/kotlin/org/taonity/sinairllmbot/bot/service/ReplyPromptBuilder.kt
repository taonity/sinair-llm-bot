...
class ReplyPromptBuilder(
    ...
    fun build(
        ...
        val system = buildString {
            ...
            append("block contains. Keep that description outside the block, followed immediately by ")
            append("the opening fence on the next line with no blank line between them. Never put the ")
            append("description inside the fence. Put the detailed content inside the block. You may add ")
            append("a short conclusion immediately on the line after the closing fence, with no blank ")
            append("line between them, only when it ")
            append("adds useful information; normally stop at the block. If the response would exceed ")
            ...
            append("as a list; use plain text inside only when a list would not fit the content. ")
